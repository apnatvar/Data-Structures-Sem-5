...
//can also choose a random larger prime number for unkown number of entries
...
void checkAndAdd(char* sampleName){
    ...
    if (hashTableTop != NULL){ // no values in hashtable hence new node must be added
        ...
        while (tempPointer != NULL){ // will look for the same surname without key as hash table is not ready for retrieval yet
    ...
    while(flag){
        ...
        while (tempPointer->next != NULL){
            if (tempPointer->key > tempPointer->next->key){
                ...
                //i was unable to find a more efficent way of swapping values
            }
            tempPointer = tempPointer->next;
        }
    }
...
void findName(char* sampleInput, char* actualInput){
    ...
    //the actual string may not be at that value and hence we have to go further
...
int main(int argc, char *argv[]){
    ...
    sortList(); // now the hash table is ready for retrieval of values based solely on keys
